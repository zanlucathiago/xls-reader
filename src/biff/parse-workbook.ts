...
// substream, then decode each sheet the globals point to. Sheets are located by
// the byte offset of their BOF, so we index records by offset once.
export function parseWorkbook(stream: Uint8Array): Workbook {
  ...
  const sheets = globals.boundSheets.map((sheet) =>
    decodeSheet(records, sheet, offsetToIndex, globals),
  );
  return { sheets };
}
